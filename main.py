...
config = {
    'deezer': {
        'cookie_arl': '3c4b24b215afa45bee789ef23f691789f589a84f844f9dbbd6e115080a2806831389a14ad3dfed6e58b6f77da3715af588e9caa0a42017c629acca110d2abcd5bf43c1b69a7c8b56b4be6c721b705f9f522e847d44dc0c5c09261ecde5988316',
        'quality': 'your_quality_value',
        'format': 'your_format_value',
        'cookie': 'your_cookie_value',
        'user_id': 'your_user_id_value',
        'user_agent': 'your_user_agent_value'
    },
...
def init_deezer_session() -> None:
    ...
    set_song_quality('mp3', web_sound_quality)
...
if __name__ == '__main__':
    ...
    # set the type based on the url that is passed, if the url contains /track/ then type is TYPE_TRACK etc
    # url = sys.argv[1]
    # url = 'https://www.deezer.com/fr/track/925108'
    url = 'https://www.deezer.com/fr/playlist/3037066082'
    typeForDownload = None
    if url.find('/trackkk/') != -1:
        typeForDownload = TYPE_TRACK
    elif url.find('/album/') != -1:
        typeForDownload = TYPE_ALBUM
    elif url.find('/playlist/') != -1:
        typeForDownload = TYPE_PLAYLIST

    # stop executing when type is still none, also print a message
    if typeForDownload == None:
        print("ERROR: could not determine type from url")
        sys.exit(1)

    # extarct the number from the url
    id = url.split('/')[-1]
    print("id: " + id)
    ...
    for song in s:
        ...
        # clean title so that it only uses char accepted by window file explorer
        cleanedTitle = re.sub(r'[<>:"/\\|?*]', '', song["SNG_TITLE"])
        download_song(song, cleanedTitle + ".mp3")
    print('All Done')
